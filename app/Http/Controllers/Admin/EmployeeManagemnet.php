...
class EmployeeManagemnet extends Controller
{
	public function EmployeeList()
	{
		...
		if($userType == 2)
		{
			$employees = DB::table('users')
				...
				->where('users.user_add_by', '=', $userId)
	...
	public function add_user_employee(Request $request)
	{
		...
		try {
			...
			$userType = Auth::user()->u_type;
			$routeLink = ($userType == 2 || $userType == 3) ? route('user.EmployeeList') : route('CA.EmployeeList');
	...
	public function downloadPayslip($id)
	{
		...
		if ($UserType == 2) {
			...
			// If admin data is not available, redirect to admin profile page
			if (!$company_data) {
	...
	public function checkCompanyPolicies()
	{
		...
		if($userType == 2)
		{
			...
			$company = Admin_profiles::where('userId', $userId)->first();
		}
