...
class ProformasController extends Controller
{
	...
	public function proformaInvoiceIndex(Request $request)
	{
		...
		if (Auth::user()->u_type != 3 && Auth::user()->u_type != 6) { //user
			$proformas = DB::table('proformas')
						->leftJoin('company_profiles', 'proformas.added_by', '=', 'company_profiles.userId')
						->leftJoin('proprietorship_profiles', 'proformas.propId', '=', 'proprietorship_profiles.id')
		...
		$proformas_pagination = $proformas;
		//echo "<pre>";print_r($proformas);exit;
		$array = array();
		foreach ($proformas as $k => $val) {
		...
		$proformas = json_decode(json_encode($array));


		//echo "<pre>"; print_r($proformas);exit;
		return view('User.proformas.sales-proforma')->with([
			'title' => $title,
			'sales' => $proformas,
			'sales_pagination' => $proformas_pagination,
			'proforma_create_status' => $this->proforma_create_status(),
		]);
	}
    ...
    public function CreateProformaInvoice()
	{
		...
		$invoiceNo = $this->generateProformaNumber($userId);
		...
		return view('User.proformas.create-proforma-invoice')->with([
			'invoiceNo' => $invoiceNo,
			'compData' => $compData,
			'comp_name' => $comp_name,
           ...
           // 'countries'=>$countries,
			...
			'proforma_create_status' => $proforma_create_status,
    ...
    protected function getProformacust(Request $request)
	{
		//echo "<pre>"; print_r($request); exit;
		$id = $request->id;
		$salesTableID = $request->salesTableID;
		$result = Proformas::query()
			->where('inv_name', '=', $id)
    ...
    protected function create(array $data)
	{
		...
		$propId = $data['propId'];
		if(!empty($propId)) {
			$invoiceNo = $this->generateProformaNumberProprietorship($propId,$userId);
		} else {
			$invoiceNo = $this->generateProformaNumber($userId);
		}
    ...
    public function save_proforma_invoice(Request $request)
	{
		if ( $this->proforma_create_status() == "false") {
			$msg = array(
				'message' => 'Unable to create proforma! Please complete your previous invoices first.'
			);
			...
			if ($validation->fails()) {
				...
				if ($insertSales) {
					$msg = array(
						'status' => 'success',
						'class' => 'succ',
						'redirect' => url('/edit-proforma-invoice/' . base64_encode($sId)),
    ...
    public function items_proforma_list($sid)
	{

		$array = array();
		$sales_values = DB::table('proformas_values')
			->select(DB::raw('proformas_values.*'))
			->where('sid', '=', $sid)
			->get();

			$sales_dataee = DB::table('proformas')
	...
	public function edit_proforma_invoice($sId)
	{
		...
		$sales_values = $this->items_proforma_list($sId);
		return view('User.proformas.edit-proforma-invoice')->with([
			'products' => $products,
			'sales' => $sales,
	...
	public function update_proforma_customer(Request $request)
	{
		$sId = $request->id;
	...
	public function view_proforma_invoice($sId)
	{
		$sId = base64_decode($sId);
		...
		$sales_values = $this->items_proforma_list($sId);
		return view('User.proformas.view-proforma-invoice')->with([
			'products' => $products,
			'sales' => $sales,
			'sales_values' => $sales_values,
			'countries' => $countries,
	...
	public function update_proforma_invoice(Request $request)
	{
	...
	{
		$custId = DB::table('proformas')
			->select(DB::raw('proformas.inv_name'))
			->where('id', '=', $sid)
			->get();
		...
		$taxableInvTotal = DB::table('proformas_values')
			->join('proformas', 'proformas.id', '=', 'proformas_values.sid')
			->where('proformas.id', '=', $sid)
			->select(DB::raw('
				SUM(proformas_values.amount - proformas_values.disc_amt) as taxableInvTotal
			'))
	...
	public function saveProductAndProforma(Request $request)
	{
		...
		try {
			...
			//INSERT INTO proformas_values
			DB::table('proformas_values')->insert([
				'sid'       => $sid, 
				'uid'       => $uid,
			...
			DB::commit();
			$proformas_values = $this->items_proforma_list($sid);
			$this->updateGstType($sid);
			return view('User.proformas.ajax-proforma-invoice-display')->with([
				'quotations_values' => $proformas_values,
			]);
	...
	public function proforma_items_display(Request $request)
	{
		//echo "<pre>"; print_r($request);exit;
		$sid = $request->sId;
		$uid = currentOwnerId();
		...
		$proformas_values = DB::table('proformas_values')
			->select(DB::raw('proformas_values.id,proformas_values.quantity'))
			->where('sid', '=', $sid)
			->where('uid', '=', $uid)
		...
		if (count($proformas_values) == 0) {
			$quantity = 1;
		} else {
			$quantity = $proformas_values[0]->quantity;
		}
		...
		if (count($proformas_values) == 0) {
			...
			$insertInvoice = DB::table('proformas_values')->insert($values);
		} else {
			$update = DB::table('proformas_values')
				->where('sid', '=', $sid)
				->where('uid', '=', $uid)
				->where('prod_id', '=', $prod_id)
				->update(
		...
		$proformas_values = $this->items_proforma_list($sid);
		//echo "<pre>"; print_r($proformas_values);exit;
		$this->updateGstType($sid);
		return view('User.proformas.ajax-proforma-invoice-display')->with([
			'quotations_values' => $proformas_values,
		]);
	...
	public function update_proforma_item_rate(Request $request)
	{
		...
		$sales_data = DB::table('proformas_values')
			->select(DB::raw('proformas_values.sid,proformas_values.quantity,proformas_values.disc,proformas_values.disc_type,proformas_values.gst_rate'))
			->where('id', '=', $request->id)
			->get();
		...
		$proformas_values = $this->items_proforma_list($sales_data[0]->sid);
		//echo "<pre>"; print_r($proformas_values);exit;
		$this->updateGstType($sid);
		return view('User.proformas.ajax-proforma-invoice-display')->with([
			'quotations_values' => $proformas_values,
	...
	public function update_proforma_invoice_final(Request $request)
	{
	...
	public function update_proforma_other(Request $request)
	{
	...
	{
		$data = DB::table('proformas')->where('id', $request->id)->first();
		$oldData = [
					'record' => [
		...
		if ($delInvoice) {
            ...
            AuditLogger::logEntry(
                ...
                description: "Proforma Invoice deleted: {$data->inv_num}",
                oldData: $oldData,
                newData: null
            );
			$msg = array(
	...
	public function update_proforma_item_quantity(Request $request)
	{
		...
		$sales_data = DB::table('proformas_values')
			->select(DB::raw('proformas_values.sid,proformas_values.rate,proformas_values.disc,proformas_values.disc_type,proformas_values.gst_rate'))
			->where('id', '=', $request->id)
			->get();
	...
	public function fetchProformaItem(Request $request)
	{
		$sid = $request->id;
		$proformas_values = DB::table('proformas_values')
			->select(DB::raw('proformas_values.*'))
			->where('proformas_values.id', '=', $sid)
			->get();
		//echo "<pre>"; print_r($proformas_values);exit;
		$array = array();
		...
		$proformas_values = (json_encode($array));
		//echo "<pre>";print_r($proformas_values);exit;
		return $proformas_values;
	}

	public function delProformaItem(Request $request)
	{
		//echo "<pre>"; print_r($_POST);exit;
		$delSalesItem = DB::table('proformas_values')->where('id', $request->id)->delete();
		$proformas_values = $this->items_proforma_list($request->sid);
	...
	public function updateProformaStatus(Request $request)
	{
		DB::table('proformas')
			->where('id', $request->id)
			->update([
	...
	public function quotationToProforma(Request $request)
    {
        ...
        try {
            ...
            $proformaData = (array) $quotation;

            unset($proformaData['id']);       // remove auto id
            unset($proformaData['created_at']);
            unset($proformaData['updated_at']);
			
			$propId = $proformaData['propId'];
			if(!empty($propId)) {
				$invoiceNo = $this->generateProformaNumberProprietorship($propId,$userId);
			} else {
				$invoiceNo = $this->generateProformaNumber($userId);
            ...
            $proformaId = DB::table('proformas')->insertGetId($proformaData);

            // Fetch quotation line items
            $quotationItems = DB::table('quotations_values')
            ...
            foreach ($quotationItems as $item) {
                ...
                $itemData['uid'] = $userId;
                $itemData['sid'] = $proformaId;
                DB::table('proformas_values')->insert($itemData);
            }
