	...
	public function uploadBank_statement(Request $request)
	{
		...
		for ($row = 1; $row <= 100; $row++) {
			...
			foreach ($rowData[0] as $index => $cellValue) {
				...
				if (preg_match('/(balance|closing balance|available balance|running balance)/', $value)) {
		...
		for ($row = $headerRow + 1; $row <= $highestRow; $row++) {
			...
			if (isset($headerMap['balance'])) {
				$balanceRaw = $rowData[$headerMap['balance']] ?? '';
				$balanceRaw = preg_replace('/[^0-9.\-]/', '', $balanceRaw);
				$currAmt = ($balanceRaw !== '') ? (float)$balanceRaw : 0;
			}
