                        ...
                        <div class="card border mb-4">
                            <div class="card-body p-3">
                                <h6 class="card-title mb-3 text-primary">
                                    <i class="ti ti-user-circle me-2"></i>Leave Information
                                </h6>
                                <div class="row g-3">
                                    <div class="col-md-12">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', function (e) {
                ...
                if (!leaveType || !leaveDuration || !reason) {
                ...
                const url = isUpdate ? `/update-leave-userEmployee/${leaveId}` : '{{ route("userEmployee.storeLeave") }}';
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function fetchWorkingDays() {
            fetch('/get-working-days-userEmployee', {
                    method: 'GET',
                    headers: {
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
    ...
    function viewLeaveDetails(leaveId) {
        fetch(`/get-leave-userEmployee/${leaveId}`, {
            method: 'GET',
            headers: {
            'Accept': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
    ...
    function deleteLeave(leaveId) {
        Swal.fire({
            ...
            preConfirm: () => {
                return fetch(`/delete-leave-userEmployee/${leaveId}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Accept': 'application/json'
            ...
            steps: [
                {
                    ...
                    intro: '<div class="text-center"><div class="welcome-tour-icon-container mb-4 d-inline-flex align-items-center justify-content-center" style="width: 90px; height: 90px; background: linear-gradient(135deg, rgba(0, 140, 173, 0.15), rgba(99, 102, 241, 0.15)); border-radius: 50%; color: #008CAD;"><i class="ti ti-calendar-event" style="font-size: 45px;"></i></div><p class="mb-0 text-secondary" style="font-size: 1.05rem;">Manage your leave applications, track pending/approved requests, and view balances.</p></div>'
                },
                {
