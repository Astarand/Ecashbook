...
<div class="pc-content">
            ...
            <div class="row align-items-center">
                <div class="col-md-12">
                    ...
                    <ul class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item" aria-current="page">Edit Inventory Expenses</li>
                    </ul>
                    <a href="javascript:void(0);" id="start-edit-inventory-expenses-tour" class="text-primary d-flex align-items-center gap-1 fw-semibold" style="font-size: 0.95rem;">
    ...
    <div class="row mb-4">
        <h3>Edit Inventory Expenses</h3>
    </div>
    <div class="card">
        <div class="card-body">
            <form action="javascript:void(0);" method="POST" name="addExpenseFrm" id="addExpenseFrm">
                <input type="hidden" name="id" id="eId" value="{{ $expense->id }}">
                @csrf
                <div class="row">
					<div class="mb-3 col-md-3">
						...
						<select name="propId" id="propId" class="form-control">
							...
							@foreach($proprietorships as $company)
								<option value="{{ $company->id }}" <?=($expense->propId == $company->id) ? 'selected' : '' ?>>
									{{ $company->comp_name }}
								</option>
							@endforeach
						</select>
                    ...
                    <div class="col-sm-4 mb-3">
                        ...
                        <select id="expense_types" name="expense_types" class="form-control" required>
                            ...
                            <option value="Freight inward / transportation" {{ $expense->expense_types == 'Freight inward / transportation' ? 'selected' : '' }}>Freight inward / transportation</option>
                            <option value="Customs duty & clearing charges" {{ $expense->expense_types == 'Customs duty & clearing charges' ? 'selected' : '' }}>Customs duty & clearing charges</option>
                            <option value="Loading & unloading" {{ $expense->expense_types == 'Loading & unloading' ? 'selected' : '' }}>Loading & unloading</option>
                            <option value="Insurance during transit" {{ $expense->expense_types == 'Insurance during transit' ? 'selected' : '' }}>Insurance during transit</option>
                            <option value="Packing & Preparation Costs" {{ $expense->expense_types == 'Packing & Preparation Costs' ? 'selected' : '' }}>Packing & Preparation Costs</option>
                            <option value="Direct Labour (Manufacturing/Processing)" {{ $expense->expense_types == 'Direct Labour (Manufacturing/Processing)' ? 'selected' : '' }}>Direct Labour (Manufacturing/Processing)</option>
                            <option value="Other Directly Attributable Costs" {{ $expense->expense_types == 'Other Directly Attributable Costs' ? 'selected' : '' }}>Other Directly Attributable Costs</option>
                            <option value="Taxes & Levies" {{ $expense->expense_types == 'Taxes & Levies' ? 'selected' : '' }}>Taxes & Levies</option>
                        </select>
                    </div>

                    <div class="col-sm-4 mb-3">
                        <label class="form-label">Expense Voucher No</label>
                        <input type="text" id="expense_voucher_no" name="expense_voucher_no" value="{{ $expense->expense_voucher_no }}"  class="form-control" placeholder="Enter Expense Voucher No">
                    </div>
                    <div class="col-sm-4 mb-3">
                        <label class="form-label">Expense Date<span class="text-danger">*</span></label>
                        <input type="date" id="expense_date" name="expense_date" value="{{ $expense->expense_date }}" required class="form-control" placeholder="Enter Expense Date">
                    </div>
                    <div class="col-sm-4 mb-3">
                        <label class="form-label">Purchase Invoice Reference No.</label>
                        <input type="text" id="purchase_invoice_ref_no" name="purchase_invoice_ref_no" value="{{ $expense->purchase_invoice_ref_no }}"  class="form-control" placeholder="Enter Purchase Invoice Reference No">
                    </div>
                    <div class="col-sm-4 mb-3">
                        <label class="form-label">Supplier / Service Provider Name</label>
                        <input type="text" id="supplier_name" name="supplier_name" value="{{ $expense->supplier_name }}"  class="form-control" placeholder="Enter Supplier / Service Provider Name">
                    </div>
                    
                    <div class="col-sm-4 mb-3">
                        <label class="form-label">Expense Amount<span class="text-danger">*</span></label>
                        <input type="text" id="expense_amount" name="expense_amount" value="{{ $expense->expense_amount }}" required class="form-control" placeholder="Enter Expense Amount">
                    </div>
                    
                    <div class="col-sm-4 mb-3">
                        <label class="form-label">Stock Location / Warehouse</label>
                        <input type="text" id="stock_location" name="stock_location" value="{{ $expense->stock_location }}"  class="form-control" placeholder="Enter Stock Location / Warehouse">
                    </div>
                    <div class="col-sm-4 mb-3">
                        <label class="form-label">Allocation Basis (Unit / Value)</label>
                        <input type="text" id="allocation_basis" name="allocation_basis" value="{{ $expense->allocation_basis }}"  class="form-control" placeholder="Enter Allocation Basis (Unit / Value)">
                    </div>
                    <div class="col-sm-4 mb-3">
                        <label class="form-label">Allocated Units / Lot No</label>
                        <input type="text" id="allocated_units" name="allocated_units" value="{{ $expense->allocated_units }}" class="form-control" placeholder="Enter Allocated Units / Lot No">
                    </div>
                    <div class="col-sm-4 mb-3">
                        <label class="form-label">Cost Allocation Amount</label>
                        <input type="text" id="cost_allocation_amount" name="cost_allocation_amount" value="{{ $expense->cost_allocation_amount }}" class="form-control" placeholder="Enter Cost Allocation Amount">
                    </div>
                    <div class="col-sm-4 mb-3">
                        <label class="form-label">Remarks</label>
                        <textarea id="remarks" name="remarks" class="form-control" placeholder="Enter Remarks" rows="3">{{ $expense->remarks }}</textarea>
                    </div>
					<div class="card">
							...
							<div class="row">
								<div class="col-md-12">
									...
									<div class="row">
										<div class="col-6">
											<div class="card shadow-sm border-0 p-3 m-2">
												<div class="form-check">
													<input class="form-check-input" type="radio" name="tds_applicable" value="yes" id="tdsYes" {{ old('tds_applicable', $expense->tds_applicable ?? '') == 'yes' ? 'checked' : '' }}>
										...
										<div class="col-6">
											<div class="card shadow-sm border-0 p-3 m-2">
												<div class="form-check">
													<input class="form-check-input" type="radio" name="tds_applicable" value="no" id="tdsNo" {{ old('tds_applicable', $expense->tds_applicable ?? '') == 'no' ? 'checked' : '' }}>
								...
								<div class="tds-container col-md-12"  id="tdsContainer">
									<div class="row">
										<div class="col-md-6">
											<div id="tds_dropdown_universal">
												...
												<select name="tds_percent" id="tds_percent" class="form-control">
													...
													<option value="{{ $purpose->tds_rate . '-' . $purpose->id }}"  {{ ($purpose->id == $expense->tds_id) ? 'selected' : '' }}>
										...
										<div class="col-md-6">
											...
											<input type="text"  id="tds_amount"  value="{{$expense->tds_amount}}" class="form-control" readonly>
										</div>
									</div>
								</div>
								...
								<div class="gst-container col-md-12">
									<div class="mb-3">
										...
										<div class="row">
											@php
												$gstApplicable = strtolower(trim($expense->gst_applicable ?? 'no'));
											@endphp
											<div class="col-6">
												<div class="card shadow-sm border-0 p-3 m-2">
													<div class="form-check">
									...
									<div class="row mb-3">
										<div class="col-md-4">
											...
											<select class="form-select" name="gst_trans" id="gst_trans">
												<option value="">Select</option>
												<option value="intrastate" <?= ($expense->gst_trans == 'intrastate')?'selected':'' ?>>Intra State</option>
												<option value="interstate" <?= ($expense->gst_trans == 'interstate')?'selected':'' ?>>Inter State</option>
												<option value="union" <?= ($expense->gst_trans == 'union')?'selected':'' ?>>Union Territory</option>
											</select>
										</div>
										<div class="col-md-4">
											<label for="gst_rate">GST Rate (%)</label>
											<input type="number" name="gst_rate" id="gst_rate" value="{{$expense->gst_rate}}" class="form-control" min="0" step="0.01">
										</div>
										<div class="col-md-4">
											<label for="gst_allocation">GST Allocation</label>
											<input type="text" name="gst_allocation" id="gst_allocation" value="{{$expense->gst_allocation}}" class="form-control" readonly>
										</div>
										<div class="col-md-4">
											<label for="gst_amt">Total GST Amount</label>
											<input type="text" name="gst_amt" id="gst_amt" value="{{$expense->gst_amt}}" class="form-control" readonly>
										</div>
										<div class="col-md-4">
											<label for="itc_applicable">ITC Applicable</label>
											<select class="form-select" name="itc_applicable" id="itc_applicable">
												<option value="no" <?= ($expense->itc_applicable == 'no')?'selected':'' ?>>NO</option>
												<option value="yes" <?= ($expense->itc_applicable == 'yes')?'selected':'' ?>>Yes</option>
											</select>
										</div>											
									</div>
								</div>
                    ...
                    <div class="col-sm-4 mb-3">
						...
						<a href="{{ asset($expense->supporting_document) }}" target="_blank">
...
<script>
    ...
    $(document).ready(function() {
        $('#start-edit-inventory-expenses-tour').on('click', function(e) {
            e.preventDefault();
            startEditInventoryExpensesTour();
        });
    });
</script>
@endsection