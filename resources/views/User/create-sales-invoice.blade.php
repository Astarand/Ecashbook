            ...
            <div class="row align-items-center">
                ...
                <div class="col-md-12">
                    <div class="page-header-title">
                        <h2 class="mb-0">Create Sales Invoice</h2>
                    </div>
                </div>
...
<div class="modal fade" id="confirmModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to convert this Proforma to Sales Invoice?</p>
            </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
		...
		//Start Quotation to sales invoice
		...
		$('#quotation_ref_num').on('change', function () {
			...
			selectedQuotation = $currentDropdown.val();
			confirmed = false;
		...
		$('#confirmYesBtn').on('click', function () {
			...
			$.ajax({
				url: "{{ route('quotation.to.sales') }}",
				type: "POST",
				data: {
					quotation_ref_num: selectedQuotation
				},
				success: function (res) {
					...
					if (res.status) {
						$('#messageModal').on('hidden.bs.modal', function () {
							window.location.href = "{{ url('sale-invoices') }}";
						});
					} else {
						$currentDropdown.prop('selectedIndex', 0);
					}
				},
				error: function () {
					$('#modalMessage').text('Server error');
					$('#messageModal').modal('show');
					$currentDropdown.prop('selectedIndex', 0);
				}
			});
		...
		$('#confirmModal').on('hidden.bs.modal', function () {
			if (!confirmed && $currentDropdown) {
				$currentDropdown.prop('selectedIndex', 0); // 🔥 reset dropdown
			}
		});
