                                    ...
                                    <div class="prod-action-links">
                                        <ul class="list-inline me-auto mb-0">
											...
											@php
												switch($v->source) {
													case 'Sales':
														$viewUrl   = url('/view-sales-invoice/'.base64_encode($v->f_id));
														$editUrl   = url('/edit-sales-invoice/'.base64_encode($v->f_id));
														$listRoute = route('user.SalesInvoices');
														break;
													case 'Purchase':
														$viewUrl   = url('/view-purchase-invoice/'.base64_encode($v->f_id));
														$editUrl   = url('/edit-purchase-invoice/'.base64_encode($v->f_id));
														$listRoute = route('user.PurchaseInvoices');
														break;
													case 'Expense':
														$viewUrl   = url('/view-expenses/'.base64_encode($v->f_id));
														$editUrl   = url('/edit-expenses/'.base64_encode($v->f_id));
														$listRoute = route('user.ExpensesList');
														break;
													case 'Income':
														$viewUrl   = url('/viewincome/'.base64_encode($v->f_id));
														$editUrl   = url('/editincome/'.base64_encode($v->f_id));
														$listRoute = route('user.OtherIncomeList');
														break;
													case 'Asset':
														$viewUrl   = url('/view-asset/'.base64_encode($v->f_id));
														$editUrl   = url('/edit-asset/'.base64_encode($v->f_id));
														$listRoute = route('user.AssetList');
														break;
													case 'Liability':
														$viewUrl   = url('/view-liabilities/'.base64_encode($v->f_id));
														$editUrl   = url('/edit-liabilities/'.base64_encode($v->f_id));
