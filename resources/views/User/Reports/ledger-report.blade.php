...
<script>
	...
	function renderTablePage() 
	{
		...
		ledgerRows.slice(start, end).forEach((r, index) => {

			const rowIndex = start + index;
			const detailId = `ledger-detail-${rowIndex}`;
			...
			*/
			...
			html += `
				<tr class="ledger-main-row">

					<td>
						<button type="button"
								class="btn btn-sm btn-light ledger-toggle"
	...
	function renderLedgerDetails(row)
	{
		let html = '';
		...
		details.forEach(detail => {
			...
			html += `
				<tr>
					...
					<td class="text-end">
						${formatINR(detail.debit || 0)}
					</td>

					<td class="text-end">
						${formatINR(detail.credit || 0)}
					</td>
	...
	$('#frmLedger').on('submit', function(e) {
		...
		$.ajax({
			...
			success: function(res) {
				...
				$('.summary-closing').text(`₹ ${formatINR(res.closing)} ${res.closing_dc}`);
			}
		});
	});
