    ...
    <div class="page-header">
        <div class="page-block">
            ...
            <div class="alert alert-danger" role="alert">
                Unable to create Proforma! Please complete your previous invoices first.
            </div>
            @endif
        </div>
    </div>
                                    ...
                                    <div class="prod-action-links">
                                        <ul class="list-inline me-auto mb-0">
                                            <li class="list-inline-item align-bottom" data-bs-toggle="tooltip"
                                                title="View Proforma">
                                                <a target="_blank" href="{{ url('/proforma-invoice-pdf/'.base64_encode($sale->id).'/proforma') }}"
                                            ...
                                            <li class="list-inline-item align-bottom" data-bs-toggle="tooltip"
                                                title="View">
                                                <a href="{{ url('/view-proforma-invoice/'.base64_encode($sale->id)) }}"
                                                    class="avtar avtar-xs btn-link-warning btn-pc-default">
                                            ...
                                            <li class="list-inline-item align-bottom" data-bs-toggle="tooltip"
                                                title="Edit">
                                                <a href="{{ url('/edit-proforma-invoice/'.base64_encode($sale->id)) }}"
...
<script>
            ...
            steps: [
                {
                    ...
                    intro: '<div class="text-center"><div class="welcome-tour-icon-container mb-4 d-inline-flex align-items-center justify-content-center" style="width: 90px; height: 90px; background: linear-gradient(135deg, rgba(0, 140, 173, 0.15), rgba(99, 102, 241, 0.15)); border-radius: 50%; color: #008CAD;"><i class="ti ti-file-analytics" style="font-size: 45px;"></i></div><p class="mb-0 text-secondary" style="font-size: 1.05rem;">Manage proforma invoices, draft agreements, payment totals, and client acceptances.</p></div>'
                },
                {
	...
	$('#updateStatus').on('click', function () {
		...
		$.ajax({
			url: "{{ route('proforma.update-proforma-status') }}",
			type: "POST",
			data: {
    ...
    $('#confirmDelete').on('click', function() {
        if (deleteId) {
			...
			$("#loader").show();
            $.ajax({
                url: '/delProformaInvoice/' + deleteId, 
                type: 'DELETE',
