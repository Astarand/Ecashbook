                                            ...
                                            <table class="table table-hover mb-0">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><input type="text" class="form-control" id="quantity_<?php echo $value->id; ?>" data-id="{{ $value->id }}" data-sid="{{ $value->sid }}" data-prod_id="{{ $value->prod_id }}" class="form-control quantity" placeholder="Quantity" value="{{ $value->quantity }}" onChange="changeQuotationQuantity(this);" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')"></td>
                                                        <td><input type="text" class="form-control" name="rate" id="rate_<?php echo $value->id; ?>" data-id="{{ $value->id }}" data-sid="{{ $value->sid }}" onChange="changeQuotationRate(this);" class="form-control rate" placeholder="Item Price" value="{{ $value->rate }}" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')"></td>
                                                        <td>₹{{ $value->disc_amt }}</td>
                                                        <td>₹{{ $value->amount }}</td>
                                                        <td class="text-center">
                                                            <!--<a href="javascript:void(0);" data-id="{{ $value->id }}" data-sid="{{ $value->sid }}" data-rate="{{ $value->rate }}" data-discamt="{{ $value->disc_amt }}" data-taxtype="{{ $value->tax_type }}" onclick="editQuotationItem(this)" class="avtar avtar-s btn-link-primary btn-pc-default" data-bs-toggle="tooltip" aria-label="Edit" data-bs-original-title="Edit"><i class="ti ti-pencil f-20"></i></a>-->
                                                            <span data-bs-toggle="tooltip" aria-label="Delete" data-bs-original-title="Delete">
                                                                <a href="javascript:void(0);" data-id="{{ $value->id }}" data-sid="{{ $value->sid }}" onclick="delQuotationItem(this)" class="avtar avtar-s btn-link-danger btn-pc-default"  data-bs-target="#delete_discount"><i class="ti ti-trash f-20"></i></a>
                                                            </span>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        $("form#addQuotationsFrmTwo").on("submit", function(e) {
            e.preventDefault();
            //if (addQuotationsFrmTwo.form()) {
            $("#loader").show();
            var itemurl = "/update_quotation_customer";
            var custData = $("form#addQuotationsFrmTwo").serialize();
      ...
      //  Product selection triggers GST data fetch
        ...
        $("form#addQuotationsFrmThree").on("submit", function(e) {
            e.preventDefault();



            // if (addQuotationsFrmThree.form()) {
            $("#loader").show();
            let signature = $("#addQuotationsFrmThree #signature").prop("files")[0];
            let signature_name = $("#addQuotationsFrmThree #signature_name").val();
            let discount_amount = $("#addQuotationsFrmThree #discount_amount").val();
            let id = $("#sId").val();
            ...
            $.ajax({
                url: base_url + "/update_quotation_invoice_final",
                type: "POST",
                data: sales_data,
                contentType: false,
                processData: false,
                success: function(response) {
                    $("#loader").hide();
                    if (response.class == "succ") {
                        $("#addQuotationsFrmThree .message-container").html(
                            '<div class="' +
                            response.class +
                            '">' +
                            response.message +
        ...
        $("form#addQuotationsFrmFour").on("submit", function(e) {
            e.preventDefault();
            $("#loader").show(); // Show the loader

            var itemurl = base_url + "/update_quotation_other";
            var custData = $("form#addQuotationsFrmFour").serialize(); // Serialize form data
	...
	function changeQuotationQuantity(el) {
		//alert('hell');
		var id = $(el).data("id");
		var sid = $(el).data("sid");
		...
		$("form#addQuotationsFrmThree #invoiceData").html("");
		$("#loader").show();
		$.ajax({
			method: "POST",
			//dataType: "json",
			url: base_url + "/update_quotation_item_quantity",
			data: { id: id, sid: sid, prod_id: prod_id, quantity: quantity },
			success: function (result) {
				$("#loader").hide();
				$("form#addQuotationsFrmThree #invoiceData").html(result);
				updateTotalAmount();
	...
	function editQuotationItem(el) {
		var id = $(el).data("id");
		var base_url = $("#base_url").val();
		$("#loader").show();
		$.ajax({
			method: "POST",
			url: base_url + "/fetchQuotationItem",
			data: { id: id },
			datatype: "json",
			success: function (result) {
				$("#loader").hide();
	...
	function changeQuotationRate(el) {
		var id = $(el).data("id");
		var sid = $(el).data("sid");
		...
		$.ajax({
			method: "POST",
			//dataType: "json",
			url: base_url + "/update_quotation_item_rate",
