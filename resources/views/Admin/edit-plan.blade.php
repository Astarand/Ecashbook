      ...
      <div class="card">
        ...
        <div class="card-body">
          <form action="javascript:void(0);" name="addSubscriptionFrm" id="addSubscriptionFrm" method="post" enctype="multipart/form-data">
           <input type="hidden" name="id" id="planId" value="{{ $plan->id }}">
					  @csrf
            ...
            <div class="row">
              <div class="col-md-3">
                <div class="form-group mb-3">
                  ...
                  <div class="d-flex align-items-center">
                    <input type="hidden" name="icon" id="selected-icon" value="{{ old('icon', $plan->icon) }}" required>
                    <div class="selected-icon-display me-2 d-flex align-items-center justify-content-center bg-light"
                        style="width: 40px; height: 40px; border-radius: 8px;">
                      <i id="selected-icon-preview" class="{{ old('icon', $plan->icon) }} fs-4"></i>
                    </div>
              ...
              <div class="col-md-3">
                <div class="form-group mb-3">
                  ...
                  <input type="text" class="form-control" name="title" placeholder="Enter plan name" value="{{ old('name', $plan->title) }}" required>
                  @error('title')
                  <div class="text-danger">{{ $message }}</div>
                  @enderror
                </div>
              ...
              <div class="col-md-2">
                <div class="form-group mb-2">
                  ...
                  <input type="number" class="form-control" name="monthly_price" placeholder="0.00" value="{{ old('monthly_price', $plan->monthly_price) }}" required>
                  @error('monthly_price')
                  <div class="text-danger">{{ $message }}</div>
                  @enderror
                </div>
              ...
              <div class="col-md-2">
                <div class="form-group mb-2">
                  ...
                  <input type="number" class="form-control" name="yearly_price" placeholder="0.00" value="{{ old('yearly_price', $plan->yearly_price) }}" required>
                  @error('yearly_price')
                  <div class="text-danger">{{ $message }}</div>
                  @enderror
                </div>
			  ...
			  <div class="col-md-2">
                <div class="form-group mb-2">
                  ...
                  <input type="number" class="form-control" name="ca_percentage" placeholder="0.00" value="{{ old('ca_percentage', $plan->ca_percentage) }}" required>
                  @error('ca_percentage')
                  <div class="text-danger">{{ $message }}</div>
                  @enderror
                </div>
                ...
                <div class="form-group mb-3">
                  ...
                  <div class="features-container">
                    ...
                    <div class="col-md-9">
						...
						<select class="form-select" name="features[{{ $index }}][feature_id]">
							<option value="">-- Select Feature --</option>
							@foreach($menuFeatures as $mf)
								<option value="{{ $mf->id }}" {{ $mf->id == $feature->feature_id ? 'selected' : '' }}> {{ $mf->code }}</option>
							@endforeach
						</select>
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="col-md-1">
                        ...
                        <button type="button" class="btn btn-danger btn-sm remove-feature"><i class="ti ti-minus"></i></button>
                        <button type="button" class="btn btn-primary btn-sm add-feature"><i class="ti ti-plus"></i></button>
                    </div>
                </div>
                    @endforeach
                  </div>
                ...
                <div class="form-group mb-3">
                  <div class="custom-switch">
                    ...
                    <div class="switch-toggle">
                      <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ $plan->status ? 'checked' : '' }}>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="row mt-3">
              <div class="col-12">
                <div class="text-end">
                  ...
                  <button type="submit" class="btn btn-primary">Update Plan <i class="ph-duotone ph-arrow-circle-up-right"></i></button>
                </div>
              </div>
            </div>
          </form>
...
<script>
	...
	const featureOptions = `
        <option value="">-- Select Feature --</option>
        @foreach($menuFeatures as $mf)
            <option value="{{ $mf->id }}">{{ $mf->code }}</option>
        @endforeach
    `;
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function reindexFeatures() {
	  ...
	  featureItems.forEach((item, index) => {
		item.querySelector('select[name*="[feature_id]"]')
			.name = `features[${index}][feature_id]`;

		item.querySelector('select[name*="[is_enabled]"]')
			.name = `features[${index}][is_enabled]`;

		const hiddenId = item.querySelector('input[name*="[id]"]');
		if (hiddenId) {
			hiddenId.name = `features[${index}][id]`;
		}
	  });
    }
    ...
    iconOptions.forEach(option => {
      option.addEventListener('click', function() {
        ...
        selectedIconPreview.className = `${iconClass} fs-4`;
    ...
    if (!selectedIconInput.value) {
     ...
     $('form#addSubscriptionFrm').bind('submit', function(e){			
      ...
      var suburl = (planId == "") ?  '/save_plan' :  '/update-plan/' + planId; // Determine URL based on plan ID
