...
function createFilePreview(file, filepath, fileName) {
...
function preview_file(el) {
    ...
    if (attachment_file) {
        ...
        if (attachment_file.size > 10 * 1024 * 1024) {
            ...
            setTimeout(function () {
                $("#chat-form .message-container div").fadeOut(
                    300,
                    function () {
                        $("#chat-form .message-container").html("");
                    }
        ...
        createFilePreview(attachment_file, null, fileName).then(
            (previewHtml) => {
                $("#file_prev_attachment_section").html(
                    previewHtml +
                        '<span class="remove-file" onclick="remove_image(event)"><i class="ti ti-x"></i></span>'
                );
...
function refreshMessages() {
    ...
    messageInputValue = $("#chat-widget-message-text").val();
    messageFileValue = $("#chat-widget-message-file").val();

    if (base_url && c_qid) {
        $.ajax({
            url: base_url + "/refresh-messages",
            method: "POST",
            data: {
                c_qid: c_qid,
            ...
            success: function (response) {
                if (response.success) {
                    $(".chatConversationSection").html(response.messages);
                    updateScroll();

                    // Restore input values after refresh
                    $("#chat-widget-message-text").val(messageInputValue);
                    $("#chat-widget-message-file").val(messageFileValue);
                }
            },
        });
    }
...
$(document).ready(function () {
    ...
    $(document).on("click", "#chat-btn", function (e) {
        ...
        if (chat_message == "" && message_file == "") {
            console.log("Both message and file are empty. Not sending.");
        ...
        $("#loader").show();
        console.log("Sending AJAX request to:", base_url + "/insert_chat");
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.class == "succ") {
                    ...
                    $("#chat-widget-message-file").val("");
                    messageInputValue = ""; // Clear stored input value
                    messageFileValue = ""; // Clear stored file value
